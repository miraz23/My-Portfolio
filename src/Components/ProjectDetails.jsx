...
const ProjectDetails = ({ activeId, setShowDetails }) => {
    useEffect(() => {
        AOS.init()
    }, [])

    const project = data.find(data => data.id === activeId)    

    return (
        <div className='w-full h-screen fixed top-0 left-0 z-10 backdrop-filter backdrop-blur-sm bg-opacity-10 font-sans'>
            ...
            transform -translate-x-1/2 -translate-y-1/2 p-5'>
                <div className='flex items-center justify-center'>
                    <figure>
                        <img className='rounded-[8px]' src={project.imgUrl} alt="project" />
                    </figure>
                </div>
                <div>
                    <h2 className='text-xl sm:text-2xl text-[#0a192f] font-[700] my-5'>{project.title}</h2>
                    <p className='text-[12px] sm:text-[15px] text-[#4d4d4d]'>{project.description}</p>
                </div>
                <div className='mt-5 flex items-center gap-3 flex-wrap'>
                    <h4 className='text-[14px] sm:text-[18px] text-[#0a192f] font-[700]'>Technologies:</h4>
                    {
